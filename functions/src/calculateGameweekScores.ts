...
import { calculatePlayerPoints, normalizePosition, PlayerMatchStats, PlayerScore } from './scoring';
...
function apiKey(): string {
  ...
  if (!key) throw new Error('FOOTBALL_API_KEY environment variable not set');
  return key;
}
...
async function apiFetch<T>(path: string): Promise<T> {
  ...
  const json = await res.json() as { errors?: Record<string, string>; response: T };
...
async function fetchFinishedFixtures(
  ...
  const fixtures = await apiFetch<ApiFixture[]>(
    `/fixtures?league=${leagueId}&season=${season}&round=${encoded}`,
  );
...
async function fetchFixturePlayers(fixtureId: number): Promise<ApiPlayerStat[]> {
  // Response is [{team, players: [{player, statistics}]}]
  const teamArrays = await apiFetch<Array<{ players: ApiPlayerStat[] }>>(
...
export async function runCalculateGameweekScores(
  data: RequestData,
  db: admin.firestore.Firestore,
): Promise<{ gameweekKey: string; playersScored: number; fixturesProcessed: number }> {
  ...
  const allScores = new Map<number, PlayerScore>(); // playerId → best score (handles dups)
  ...
  for (let i = 0; i < fixtures.length; i++) {
    ...
    console.log(
      `  Fixture ${i + 1}/${fixtures.length}: id=${fId} ` +
    ...
    let playerEntries: ApiPlayerStat[];
    try {
      playerEntries = await fetchFixturePlayers(fId);
    } catch (err) {
      console.warn(`  ⚠️  Could not fetch players for fixture ${fId}: ${(err as Error).message}`);
      if (i < fixtures.length - 1) await sleep(RATE_DELAY);
      continue;
    }
    ...
    for (const entry of playerEntries) {
      ...
      const rawEntry = entry as ApiPlayerStat & { statistics: Array<{ team?: { id: number } }> };
      ...
      const score = calculatePlayerPoints(mapped, leagueId);
  ...
  console.log(
    `[calculateGameweekScores] Done — ${allScores.size} players scored ` +
