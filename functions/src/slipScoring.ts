...
import { SlipPick, PickType, MatchResultCache } from './slipTypes';

// ─── Points constants ─────────────────────────────────────────────────────────

export const PTS: Record<PickType, number> = {
  result:        10,
  overUnder:      8,
  btts:           8,
  correctScore:  25,
...
function scoreResult(value: string, result: MatchResultCache): boolean {
  if (result.homeGoals > result.awayGoals) return value === 'home';
  if (result.awayGoals > result.homeGoals) return value === 'away';
  return value === 'draw';
}

/** value format: 'over_2.5' | 'under_3.5' */
function scoreOverUnder(value: string, result: MatchResultCache): boolean {
...
/** value: 'yes' | 'no' */
function scoreBtts(value: string, result: MatchResultCache): boolean {
  const actual = result.btts ? 'yes' : 'no';
  return value === actual;
}

/** value: '2-1' | '0-0' — goals only, no AET */
function scoreCorrectScore(value: string, result: MatchResultCache): boolean {
  const [h, a] = value.split('-').map(Number);
  return h === result.homeGoals && a === result.awayGoals;
}

/** value: playerId string */
function scoreFirstScorer(value: string, result: MatchResultCache): boolean {
...
/** value: playerId string */
function scoreAnytimeScorer(value: string, result: MatchResultCache): boolean {
 ...
 */
function scoreHtft(value: string, result: MatchResultCache): boolean {
...
function actualValue(type: PickType, result: MatchResultCache): string {
  switch (type) {
    case 'result': {
 ...
 */
export function scoreOnePick(pick: SlipPick, result: MatchResultCache): SlipPick {
  let correct = false;

  switch (pick.type) {
    case 'result':        correct = scoreResult(pick.value, result);        break;
    case 'overUnder':     correct = scoreOverUnder(pick.value, result);     break;
    case 'btts':          correct = scoreBtts(pick.value, result);          break;
    case 'correctScore':  correct = scoreCorrectScore(pick.value, result);  break;
    case 'firstScorer':   correct = scoreFirstScorer(pick.value, result);   break;
    case 'anytimeScorer': correct = scoreAnytimeScorer(pick.value, result); break;
    case 'htft':          correct = scoreHtft(pick.value, result);          break;
  }

  return {
    ...pick,
    correct,
    pointsEarned: correct ? PTS[pick.type] : 0,
 ...
 *  - Otherwise   → 1.0×
 */
export function parlayMultiplier(picks: SlipPick[]): number {
  const scored    = picks.filter((p) => p.pointsEarned !== undefined);
  if (scored.length === 0) return 1;
  const correct   = scored.filter((p) => p.correct === true).length;
  if (correct === scored.length) return 2;
  if (correct >= 5) return 1.5;
  return 1;
}

// ─── Slip scorer ──────────────────────────────────────────────────────────────

export interface ScoredSlip {
  picks:            SlipPick[];
  rawPoints:        number;
  parlayMultiplier: number;
  totalPoints:      number;
  /** How many picks have been scored (have a result) */
  scoredCount:      number;
  allScored:        boolean;
}
...
export function scoreSlip(
  picks:            SlipPick[],
  resultsByMatchId: Map<string, MatchResultCache>,
  totalPickCount:   number,
): ScoredSlip {
  const scored = picks.map((pick) => {
    const result = resultsByMatchId.get(String(pick.matchId));
    if (!result || !result.finished) return pick;  // not finished yet
