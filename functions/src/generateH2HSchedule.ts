...
import { H2HLeague, H2HMatchup } from './slipTypes';

export interface GenerateH2HScheduleRequest {
  leagueId:  string;
  /** Ordered list of gameweek keys to schedule into, e.g. ["39_GW01_2025", …] */
  gameweeks: string[];
}

export interface GenerateH2HScheduleResponse {
  leagueId:  string;
  rounds:    number;
  matchups:  number;
}

export async function runGenerateH2HSchedule(
  request: CallableRequest<GenerateH2HScheduleRequest>,
  db: admin.firestore.Firestore,
): Promise<GenerateH2HScheduleResponse> {
  ...
  if (!Array.isArray(gameweeks) || gameweeks.length === 0) {
    throw new HttpsError('invalid-argument', 'gameweeks array required.');
  }

  // ── Load league ────────────────────────────────────────────────────────────
  const leagueSnap = await db.collection('h2hLeagues').doc(leagueId).get();
  if (!leagueSnap.exists) throw new HttpsError('not-found', 'H2H league not found.');

  const league = leagueSnap.data() as H2HLeague;

  // Only league creator (first member = creator) or admin can generate schedule
  const isAdmin = request.auth.token.admin === true;
  const isCreator = league.members?.[0] === uid;
  if (!isAdmin && !isCreator) {
    throw new HttpsError('permission-denied', 'Only the league creator can generate the schedule.');
  }

  const members = league.members ?? [];
  if (members.length < 2) {
    throw new HttpsError('failed-precondition', 'Need at least 2 members to generate a schedule.');
  }
  ...
  await db.collection('h2hLeagues').doc(leagueId).set(
    { schedule, standings, updatedAt: now },
    { merge: true },
  );
...
function bergerRoundRobin(members: string[], gameweeks: string[]): H2HMatchup[] {
  const teams = [...members];
