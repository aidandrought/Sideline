...
// ─── AppMatch type (subset written to Firestore) ───────────────────────────────
...
export interface AppMatch {
  ...
  league: string;
  leagueLogo?: string;
  score?: string;           // e.g. "2-1" for finished/live
  status: 'live' | 'upcoming' | 'finished';
  minute?: string;          // e.g. "73'"
  date: string;             // ISO
  venueName?: string;
}

export interface MatchDetailCacheDoc {
  fixtureId: number;
...
function formatFixture(f: any, status: AppMatch['status']): AppMatch | null {
  const home = f?.teams?.home;
  const away = f?.teams?.away;
  const fix  = f?.fixture;
  const lg   = f?.league;

  if (!home?.name || !away?.name || !fix?.id) return null;
  ...
  if (isBlocked(home.name) || isBlocked(away.name) || isBlocked(lg?.name ?? '')) return null;
  if (isNationalTeamFixture && !isWorldCupFixture) return null;
...
const mapFixtureStatus = (fixture: any): AppMatch['status'] => {
...
async function upsertMatchDetailCache(
  ...
  const payload: MatchDetailCacheDoc = {
    fixtureId,
...
async function readExistingDetail(
  db: admin.firestore.Firestore,
  fixtureId: number,
): Promise<{ fixture?: any; events?: any[]; statistics?: any[]; lineups?: any[] } | null> {
  try {
...
async function fetchLiveFixtureDetail(
  ...
  apiKey: string,
  fixture: any,
): Promise<{ events: any[]; statistics: any[]; lineups: any[] }> {
  const fixtureId = Number(fixture?.fixture?.id);
...
async function mergeFinishedMatchesIntoResults(
  db: admin.firestore.Firestore,
  matches: AppMatch[],
): Promise<void> {
  ...
  const existing = (snap.data()?.matches || []) as AppMatch[];
  const byId = new Map<number, AppMatch>();

  matches.forEach((match) => byId.set(match.id, match));
  existing.forEach((match) => {
    if (!match?.id || byId.has(match.id)) return;
...
async function hasLikelyLiveGames(db: admin.firestore.Firestore, prevLiveCount: number): Promise<boolean> {
  // If games were live last run, keep polling until they clear
  if (prevLiveCount > 0) return true;
  ...
  try {
    ...
    const upcoming = (snap.data()?.matches ?? []) as AppMatch[];
...
export async function runFetchLive(
  db: admin.firestore.Firestore,
  apiKey: string,
): Promise<number> {
  const previousLiveSnap = await db.collection('appData').doc('live').get();
  const previousLive = ((previousLiveSnap.data()?.matches || []) as AppMatch[])
  ...
  for (const f of fixtures) {
    if (!ALL_COMPETITION_IDS.includes(f?.league?.id)) continue;
  ...
  const finalizedMatches: AppMatch[] = [];

  await Promise.allSettled(
    candidatesToFinalize.map(async (match) => {
      try {
        const fixtureData = await apiFetch(`/fixtures?id=${match.id}`, apiKey);
        const fixture = fixtureData?.response?.[0];
        if (!fixture) return;
        const short = String(fixture?.fixture?.status?.short || '').toUpperCase();
        if (!TERMINAL_STATUSES.has(short)) return;
...
const isMatchDetailFresh = (data: any): boolean => {
...
export async function runFetchMatchDetail(
  db: admin.firestore.Firestore,
  apiKey: string,
  fixtureId: number,
  options: { force?: boolean } = {},
): Promise<{ fixtureId: number; cacheHit: boolean; refreshed: boolean; status: AppMatch['status'] | null }> {
  ...
  if (!options.force && isMatchDetailFresh(existingData)) {
    return {
      ...
      cacheHit: true,
      refreshed: false,
      status: (existingData?.status as AppMatch['status']) ?? null,
    };
  }
...
export async function runFetchUpcoming(
  db: admin.firestore.Firestore,
  apiKey: string,
): Promise<number> {
  const dates: string[] = [];
  ...
  for (let i = 0; i < dates.length; i += 5) {
    ...
    const results = await Promise.allSettled(
      batch.map((date) => apiFetch(`/fixtures?date=${date}&status=NS`, apiKey))
    );
...
export async function runFetchResults(
  db: admin.firestore.Firestore,
  apiKey: string,
): Promise<number> {
  const dates: string[] = [];
  ...
  const results = await Promise.allSettled(
    dates.map((date) => apiFetch(`/fixtures?date=${date}&status=FT-AET-PEN`, apiKey))
  );
  for (const r of results) {
    if (r.status === 'fulfilled') allFixtures.push(...(r.value?.response ?? []));
  }
