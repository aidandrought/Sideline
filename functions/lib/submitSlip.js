...
exports.runSubmitSlip = runSubmitSlip;
exports.runCacheMatchResult = runCacheMatchResult;
...
async function runSubmitSlip(request, db) {
    if (!request.auth)
    ...
    if (!competition)
        throw new https_1.HttpsError('invalid-argument', 'competition is required.');
    if (mode !== 'solo' && mode !== 'h2h') {
        throw new https_1.HttpsError('invalid-argument', 'mode must be "solo" or "h2h".');
    }
    if (mode === 'h2h' && (!challengedUserId || !leagueId)) {
        throw new https_1.HttpsError('invalid-argument', 'H2H slips require challengedUserId and leagueId.');
    }
    if (!Array.isArray(rawPicks) || rawPicks.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'At least one pick is required.');
    }
    if (rawPicks.length > 10) {
        throw new https_1.HttpsError('invalid-argument', 'Maximum 10 picks per slip.');
    }
    // ── Validate each pick ────────────────────────────────────────────────────
    const seenPickKeys = new Set();
    for (const p of rawPicks) {
        if (!p.matchId)
            throw new https_1.HttpsError('invalid-argument', 'Each pick must have a matchId.');
        if (!VALID_TYPES.has(p.type)) {
            throw new https_1.HttpsError('invalid-argument', `Unknown pick type "${p.type}".`);
        }
        if (!p.value || typeof p.value !== 'string') {
            throw new https_1.HttpsError('invalid-argument', `Pick for match ${p.matchId} is missing a value.`);
        }
        const dupKey = `${p.matchId}::${p.type}`;
        if (seenPickKeys.has(dupKey)) {
            throw new https_1.HttpsError('invalid-argument', `Duplicate ${p.type} pick for match ${p.matchId}.`);
        }
        seenPickKeys.add(dupKey);
    }
    // ── Check kickoff status for each pick's match ────────────────────────────
    const matchIds = [...new Set(rawPicks.map((p) => String(p.matchId)))];
    const matchResultSnaps = await db.getAll(...matchIds.map((id) => db.collection('matchResultCache').doc(id)));
    const kickedOffIds = new Set();
    for (const snap of matchResultSnaps) {
        ...
        if (data.finished)
            kickedOffIds.add(snap.id);
    }
    // Also check client-supplied kickoffAt as a secondary gate
    const now = Date.now();
    for (const p of rawPicks) {
        if (kickedOffIds.has(String(p.matchId))) {
            throw new https_1.HttpsError('failed-precondition', `Match ${p.matchId} has already kicked off — picks cannot be changed.`);
        }
        if (p.kickoffAt && new Date(p.kickoffAt).getTime() <= now) {
            throw new https_1.HttpsError('failed-precondition', `Match ${p.matchId} kickoff time has passed.`);
        }
    }
    ...
    if (mode === 'h2h' && leagueId && challengedUserId) {
        ...
        if (!leagueSnap.exists) {
            throw new https_1.HttpsError('not-found', `H2H league "${leagueId}" not found.`);
        }
        const leagueData = leagueSnap.data();
        const members = leagueData.members ?? [];
        if (!members.includes(uid)) {
            throw new https_1.HttpsError('permission-denied', 'You are not a member of this H2H league.');
        }
        if (!members.includes(challengedUserId)) {
            throw new https_1.HttpsError('invalid-argument', 'The challenged user is not in this league.');
        }
    }
    ...
    if (existingSnap.exists) {
        ...
        if (existing.locked) {
            throw new https_1.HttpsError('failed-precondition', 'Your slip is locked — it cannot be changed after the first match kicks off.');
        }
        if (existing.status === 'scored') {
            throw new https_1.HttpsError('failed-precondition', 'This slip has already been scored.');
        }
    }
    ...
    const picks = rawPicks.map((p) => ({
        ...
        type: p.type,
        value: p.value.trim(),
        pointsAvailable: slipScoring_1.PTS[p.type], // always use server-side PTS, ignore client value
    }));
...
async function runCacheMatchResult(request, db) {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Must be signed in.');
...
//# sourceMappingURL=submitSlip.js.map