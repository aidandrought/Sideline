...
exports.TOURNAMENT_COMPETITION_IDS = void 0;
exports.calculatePlayerPoints = calculatePlayerPoints;
exports.normalizePosition = normalizePosition;
// ─── Tournament competition IDs that award the 1.5× goal/assist multiplier ───
exports.TOURNAMENT_COMPETITION_IDS = new Set([
    1, // FIFA World Cup
...
function calculatePlayerPoints(stats, competitionId) {
    ...
    const isTournament = competitionId !== undefined && exports.TOURNAMENT_COMPETITION_IDS.has(competitionId);
    // ── Goals ─────────────────────────────────────────────────────────────────
    if (stats.goals > 0) {
        // Per spec: GK=10, DEF=6, MID=6, FWD=6
        const baseGoalBonus = pos === 'GK' ? 10 :
    ...
    if (stats.cleanSheet) {
        const csBonus = pos === 'GK' || pos === 'DEF' ? 4 :
            pos === 'MID' ? 1 :
                0; // FWD — no CS bonus
        if (csBonus > 0) {
            bonuses.push({
...
function normalizePosition(apiPosition) {
    ...
    if (p.includes('goal'))
        return 'GK';
    if (p.includes('defend'))
        return 'DEF';
    if (p.includes('midfield') || p === 'mid')
        return 'MID';
