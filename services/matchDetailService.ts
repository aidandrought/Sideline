...
import { getOrFetchCached, withTimeout } from './cacheService';
...
class MatchDetailService {
  ...
  private async fetch(endpoint: string) {
    try {
      const response = await withTimeout(fetch(`${this.baseURL}${endpoint}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data?.errors && typeof data.errors === 'object' && Object.keys(data.errors).length > 0) {
          console.warn('[MatchDetailService] API error in response body:', JSON.stringify(data.errors), 'endpoint:', endpoint);
          return null;
        }
        return data;
      } else {
        console.warn('[MatchDetailService] HTTP error:', response.status, 'endpoint:', endpoint);
      }
    } catch (error) {
      if (!isTransientNetworkError(error)) {
        console.error('Match detail API error:', error, 'endpoint:', endpoint);
      }
    }
    return null;
  }
   ...
   */
  async getTeamNews(teamId: number, season?: number): Promise<TeamNews> {
    if (!Number.isFinite(teamId) || teamId <= 0) {
      return { team: '', injuries: [], suspensions: [], doubtful: [] };
    }

    try {
      const resolvedSeason = typeof season === 'number' ? season : new Date().getFullYear();
      const data = await this.fetch(`/injuries?team=${teamId}&season=${resolvedSeason}`);
      ...
      if (data?.response) {
        ...
        return {
          team: data.response[0]?.team?.name || '',
          injuries,
