...
export default function CommunityChatScreen() {
  ...
  const resolvedType: 'team' | 'league' | 'worldcup' =
    rawType === 'league' ? 'league' : rawType === 'worldcup' ? 'worldcup' : 'team';
  ...
  const communityIconName = (community?.type ?? resolvedType) === 'team' ? 'shield' : 'trophy';
  const chatDisabled = resolvedType === 'worldcup' || !!community?.isNationalTeam;

  const promptAuth = () => {
    Alert.alert('Sign in to chat', 'Create an account or log in to send messages and react.', [
  ...
  useEffect(() => {
    ...
    const unsubscribeChat = chatService.subscribeToChat(id as string, setMessages);
    return unsubscribeChat;
  }, [id, userProfile]);

  useFocusEffect(
    React.useCallback(() => {
      if (!userProfile || !id || chatDisabled) return;
      ...
      const heartbeat = setInterval(() => {
        void presenceService.heartbeat('community', roomId, userProfile.uid);
      }, 25000);

      void presenceService.enterRoom('community', roomId, userProfile.uid, displayName);

      return () => {
        clearInterval(heartbeat);
        void presenceService.leaveRoom('community', roomId, userProfile.uid);
      };
    }, [id, userProfile, chatDisabled])
  );
  ...
  const addReaction = async (emoji: string) => {
    ...
    try {
      ...
      console.error('Failed to toggle community reaction:', error);
  ...
  const renderMessage = (msg: ChatMessage, index: number, allMsgs: ChatMessage[]) => {
    const isCurrentUser = msg.userId === userProfile?.uid;
    const isSystem = msg.type === 'system';
    const showDivider = index > 0 && allMsgs[index - 1].userId !== msg.userId;

    if (isSystem) {
      return (
        <View key={msg.id}>
          {showDivider && <View style={styles.messageDivider} />}
        ...
        >
          <View style={[
            ...
            <View style={[
              ...
              { backgroundColor: isCurrentUser ? (community?.color || '#0066CC') : '#232323' }
            ]}>
              <Text style={[styles.messageText, isCurrentUser && styles.currentUserMessageText]}>{msg.text}</Text>
            </View>
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.emptyChat}>
          <Ionicons name="time-outline" size={48} color="#8E8E93" style={styles.emptyChatIcon} />
          <Text style={styles.emptyChatTitle}>Watch parties only</Text>
          <Text style={styles.emptyChatSubtitle}>
            Chat is available only during live matches.
          </Text>
        </View>
      </View>
    );
      ...
      <View style={[styles.header, { backgroundColor: community?.color || '#0066CC' }]}>
        ...
        <View style={styles.headerInfo}>
          ...
          <View style={styles.headerStats}>
            ...
            <Text style={styles.activeCount}>Community chat</Text>
          </View>
        </View>
